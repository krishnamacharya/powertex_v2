...
export class GlobalServiceService {
  ...
  getDatawithdelete(methodName, param1) {
    return this.http.delete(
      this.posturl + methodName + '?transID=' + param1,
      this.options
    );
  }
  ...
  pdfpostData(body, methodName) {
    return this.http.post(this.posturl + methodName, body, {
      responseType: 'blob',
    });
  }
  postDataWithParams(
  ...
  getDatawithQueryParamsBrands(
    paramType: string,
    value: string
  ): Observable<any> {
    const queryParam = `${encodeURIComponent(paramType)}=${encodeURIComponent(
      value
    )}`;
  ...
  getNewarrivals(endpoint: string) {
    return this.http.get(this.UrlData(`${endpoint}`), {
  ...
  getCouponDetails(couponCode: string, customerId: number, orderValue: number) {
    ...
    const url = this.UrlData(
      `getcoupons/?id=${encodedCoupon}&customerid=${customerId}&ordervalue=${orderValue}`
    );
    return this.http.get(url);
  }
  ...
  getDatawithMethodParams2dates(methodName, param1, param2) {
    return this.http.get(
      this.posturl + methodName + '?fromdate=' + param1 + '&todate=' + param2,
      this.options
    );
  }
  ...
  getDatawithQueryPar(methodName, input_id) {
    return this.http.get(
      this.posturl + methodName + '?invqhid=' + input_id,
      this.options
    );
  }
  ...
  getDatawithQueryPars(methodName, input_id) {
    return this.http.get(
      this.posturl + methodName + '?pid=' + input_id,
      this.options
    );
  }
