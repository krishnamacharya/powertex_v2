...
@Component({
  selector: 'app-customer-ledger-print',
  standalone: false,
  templateUrl: './customer-ledger-print.component.html',
  styleUrls: ['./customer-ledger-print.component.scss']
})
...
balance:any
  ...
  ngOnInit() {
    ...
    this.Headerdetails = JSON.parse(localStorage.getItem('Headerdetails'));
    if(localStorage.getItem('totalamounts')){
    this.totalamounts = JSON.parse(localStorage.getItem('totalamounts'));
    if(this.totalamounts[0].openingbalance<0){
     
      this.amount=JSON.stringify(this.totalamounts[0].openingbalance)
      this.splitbalance=this.amount.split('-')
      this.totalamounts[0].Amounts=JSON.parse(this.splitbalance[1])
      console.log( this.totalamounts," this.totalamounts")
    }
    if(this.totalamounts[0].balanceTotal<0){

      this.credit=true
      this.balance=JSON.stringify(this.totalamounts[0].balanceTotal).split('-')
      this.splitbalance=this.amount.split('-')
      this.totalamounts[0].balanceTotal=JSON.parse(this.splitbalance[1])
    }
    else{
      this.credit=false
    }
    this.totalamounts[0].balanceTotal= Math.round(this.totalamounts[0].balanceTotal);  

    }
   this.fromactiveusers=localStorage.getItem('fromactiveusers')
  }
  printPL() {
    let popupWinindow
