...
selected: any;
catg: any;
  constructor(private DatePipe: DatePipe,private service: GlobalServiceService, public route: ActivatedRoute, private dat_s: DataServiceService, private router: Router, private toasterService: ToasterService, private dialog: MatDialog, private eventemit: ComponentCommunicationService, private spinner: NgxSpinnerService) {
    this.cartobj['id'] = 1;
  }
  branchcode: any
  obj: any
  ngOnInit() {
    ...
    this.service.getHeaderDetails(1, this.loginUserData.user_id).subscribe((data) => {
      ...
      else if (this.loginUserData.role == "Employee") {
        this.headerData = {
          document_no: 1,
          document_date: formatDate(this.today, 'yyyyMMdd', 'en-US'),
          dest_user_id: this.headeDetails.handling_company_wh_manager,
    ...
    this.getprodimg();
    if (this.loginUserData.role != "Employee") {
  ...
  disp1(resp: Response): any {
    ...
    for (let k of this.h) {
      ...
      this.newAttribute1['discount1'] = k.discount_a;
      this.newAttribute1['mrp'] = k.mrp;
      this.newAttribute1['mdiscount'] = k.mdiscount;
      this.newAttribute1['piecepercarton'] = k.piecepercarton;
      this.newAttribute1['document_date'] = this.headerData.document_date;
      this.newAttribute1['created_user_id'] = this.loginUserData.user_id;
      this.newAttribute1['document_no'] = this.headerData.document_no;
...
if (this.newAttribute1.carton == "") {
...
}else if(!this.newAttribute1.tot_value){
  this.toasterService.warning ("Please Fill All The Required Fields")
}
else {
  ...
  if (this.count == 0) {
    ...
    if(!this.newAttribute1.carton){
      this.newAttribute1.carton = 0;
    }
    if(!this.newAttribute1.tot_value){
      this.newAttribute1.tot_value = 0;
    }
    this.fieldArray.push(this.newAttribute1);
  ...
  add_disable(attr) {
    let arr = ["category", "carton", "subcategory", "modelno", "qty", "piecepercarton", "net_price", "mdiscount"];
    for (let i of arr) {
      if (attr[i] == null || attr[i] == undefined || attr[i] == '') {
...
// ============================================
  ...
  onKey(catg) {
    ...
    return this.service.getDatawithQueryParams4User_id("121", catg.productid, catg.qty, this.Spl_netprice, catg.discount1, this.loginUserData.user_id).subscribe(data => {
      ...
      // catg.net_price = (catg.mrp / 100) * catg.mdiscount
...
discountQ(newAttribute1){
  // let tot_valuee = (newAttribute1.qty * newAttribute1.net_price);
  let tot_valuee = (newAttribute1.qty * newAttribute1.mrp);
  // let disc_amt = (tot_valuee * newAttribute1.mdiscount)/100;
  newAttribute1.disc_amt = (tot_valuee * newAttribute1.mdiscount)/100;
  newAttribute1.net_price = newAttribute1.mrp - ((newAttribute1.mrp / 100) * newAttribute1.mdiscount)
  // newAttribute1.tot_value = tot_valuee - disc_amt;
  newAttribute1.tot_value = tot_valuee - newAttribute1.disc_amt;
}
  ...
  confirm_order() {
    ...
    this.headerData.line_amount = this.response.price
    ...
    if (this.loginUserData.role == "Employee") {
      ...
      this.headerData.dest_company_code = this.headeDetails.company_code;
    ...
    this.QuotData = [];
    // this.spinner.show();
console.log(this.po,'poo')
for (var i = 0; i < this.po.length; i++) {
    this.QuotData.push({
      customerid : this.selectedObj.user_id,
      date : this.curDate,
      productid : this.po[i].productid,
      rate : this.po[i].net_price,
      mrp : this.po[i].mrp,
      disp : this.po[i].mdiscount,
      piceperctn : this.po[i].piecepercarton,
      disa : this.po[i].disc_amt,
      gstp : 18,
      qty : this.po[i].qty,
      gsta : (this.po[i].tot_value * 18)/100,
      total : this.response.finalamount,
      taxable : this.response.price,
      totalgst : this.response.gst,
      netamount : this.response.finalamount,
     })
}
console.log(this.QuotData,'QuotData')
    this.loginMethod = 'sup/quotation/';

    // this.body = {  "json_dtl": this.QuotData };

    // this.myJSON = JSON.stringify(this.body);
    this.myJSON = JSON.stringify(this.QuotData);
    this.service.postData(this.myJSON, this.loginMethod).subscribe((data) => {
    this.gotoPrint('');
    this.poprint = data;
      localStorage.setItem('poprintDataApi', JSON.stringify(data));
  ...
  gotoPrint(m) {
    ...
    let document_no_out = '';
    let message_out = '';
      this.body = { "comp_addr": this.selectedObj, "process_in": process_in, "json_hdr": this.headerData, "json_dtl": this.po, "operation_in": operation_in, "draft_final_in": draft_final_in, "document_no_out": document_no_out, "message_out": message_out };
    this.data1 = {"data":this.body};
      localStorage.setItem('poprintData', JSON.stringify(this.data1));
      this.router.navigateByUrl('/Quotation-Print');
    }
