...
@Component({
  selector: 'app-seependingpo-list',
  standalone: false,
  templateUrl: './seependingpo-list.component.html',
  styleUrls: ['./seependingpo-list.component.scss']
})
export class SeependingpoListComponent implements OnInit {
  token: string;
  loginUserData: any;
  math = Math;
  param1: any;
  ...
  printreport(){
    localStorage.setItem('getinvoicelistcount1', JSON.stringify(this.getinvoicelistcount1));
    this.router.navigate(['/seependingpo-list-print'])
  }
  ...
  view(packing_one) {
    ...
    this.globalservive.getDatawithQueryParams4(3.9, 5, packing_one.po_no, packing_one.po_date, packing_one.po_origin_company_code).subscribe((data) => {
      this.spinner.hide();
