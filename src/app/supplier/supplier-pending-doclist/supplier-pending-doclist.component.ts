...
declare var $: any;
@Component({
  selector: 'app-supplier-pending-doclist',
  standalone: false,
  templateUrl: './supplier-pending-doclist.component.html',
  styleUrls: ['./supplier-pending-doclist.component.scss']
})
export class SupplierPendingDoclistComponent implements OnInit {
  loginUserData: any
  supplierdata: any = []
  p: any = 1
  searchText: any
  ...
  getinvoicedata() {
    this.spinner.show();
    this.globalServicce.getDatawithMethodParams1("sup/pendinggeninvoices/", this.loginUserData.user_id).subscribe((data) => {
  ...
  uploadMore(data) {
    ...
    if (this.loginUserData.role == 'Admin') {
      this.route.navigateByUrl('Admin-Invoice-fileupload');
    } else {
      this.route.navigateByUrl('/Supplier-Invoice-fileupload?S=Doc');
    }
