...
export class GroupInvoiceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // this.pro.currency = this.loginUserData.currency
    this.pro.discount = 0;
    this.pro.fright = 0;
  ...
  getproductsdata() {
    this.productdetail = []
    this.spinner.show();
    this.globalServicce.getDatawithMethod1("sup/getinvoicedata/").subscribe((data) => {
      this.data1 = data;
  ...
  onSubmit(form: NgForm) {
    if (this.searchText) {
      // this.toasterService.warning("Clear Search");
      alert('Clear Search');
      return false;
    }
    // if()
  ...
  atributesData(form) {
    ...
    this.headerdata.supplierid = this.loginUserData.user_id
  ...
  updatemodal() {
    this.route.navigateByUrl('Supplier-Proforma Invoice Print?S=CPP');
  }
  add_disable(attr) {
  ...
  selectedlist: any = []
  qtychange(product) {
    let tnum = product.qty = Math.ceil(JSON.parse(product.qty?product.qty:0));
    console.log(tnum);
    product.amount = JSON.parse(product.qty) * JSON.parse(product.price)
    ...
    if(this.productdetail.length > 0){
      this.pro.poamount = this.productdetail.map(a => parseFloat(a.amount?a.amount:'0')).reduce(function (a, b) {
        return a + b;
      });
    }
  }
