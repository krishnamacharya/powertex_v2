...
@Component({
  selector: 'app-cha-correction',
  standalone: false,
  templateUrl: './cha-correction.component.html',
  styleUrls: ['./cha-correction.component.scss']
})
export class ChaCorrectionComponent implements OnInit {
  loginUserData: any;
  supplierdata: any = [];
  p: any = 1
  searchText: any
  ...
  constructor(private globalServicce: GlobalServiceService, private toasterService: ToasterService, private router: Router, private dialog: MatDialog, private spinner: NgxSpinnerService) { }
  ...
  getinvoicedata() {
    this.spinner.show();
    this.globalServicce.getDatawithMethodParams2("sup/documenton/",'','').subscribe((data: any) => {
      if(data){
