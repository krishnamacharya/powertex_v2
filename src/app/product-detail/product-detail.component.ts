...
export class ProductDetailComponent implements OnInit {
  ...
  PostRating(rating, title, comments): Subscription | void {
    ...
    if (this.token != '' && this.token != undefined) {
      ...
      this.ratebody = {
        ...
        title: title,
        comments: comments,
        sno: this.proddetails.seq_no,
        productid: this.proddetails.productid,
        userid: this.loginUserData.user_id,
      };
      this.ratemethod = 'review/';
      /* alert(rating + "-----" + title + "-----" + comments) */
      return this.globalService
        .postData(this.ratebody, this.ratemethod)
        .subscribe((data) => {
        ...
        (resp: any) => {
          ...
          for (var i = 0; i < this.reviewdetails.releted_products.length; i++) {
            if (this.reviewdetails.releted_products[i]) {
              var text =
                this.reviewdetails.releted_products[i].Image.split(
                  'Powertexmodel'
                );
              this.reviewdetails.releted_products[i].Image =
                'https://gstbucket1.s3.ap-south-1.amazonaws.com/Powertexmodel' +
                text[1];
            }
          }
  ...
  getattribute() {
    // this.spinner.show();
    return this.globalService
      .getDatawithQueryParams1('3.8', this.subcatg)
      .subscribe(
        (resp) => {
          // this.spinner.hide();
          console.log('attr', resp);
  ...
  dis_cart() {
    ...
    if (
      this.loginUserData.user_type != 'Customer' &&
      this.loginUserData.user_type != 'Guest' &&
      this.loginUserData.user_type != 'Dealer' &&
      this.loginUserData.user_type != 'Employee' &&
      this.loginUserData.user_type != 'Speciall'
    ) {
      return true;
    } else {
      return false;
  ...
  addToWishlist(details: any) {
    ...
    // You can call your wishlist service here as well
    // this.wishlistService.add(details);
  }
