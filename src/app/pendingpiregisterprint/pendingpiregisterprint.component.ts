...
@Component({
  selector: 'app-pendingpiregisterprint',
  standalone: false,
  templateUrl: './pendingpiregisterprint.component.html',
  styleUrls: ['./pendingpiregisterprint.component.scss']
})
export class PendingpiregisterprintComponent implements OnInit {
  searchText:any;
  p: any = 1
  loginUserData: any;
  SupplierDtls: any;
  ...
  suppliers: any = []
  constructor(private route: Router,private router: Router, private dialog: MatDialog, private spinner: NgxSpinnerService) { }
  ...
  getOrdersummery() {
    ...
    this.PendingPayments1 = JSON.parse(localStorage.getItem('PendingPayments'));
  };
  PendingPayments1: any = [];
  searchdata(data) {
    if (data) {
      this.PendingPayments = []
      for (var i = 0; i < this.PendingPayments1.length; i++) {
        if ((this.PendingPayments1[i].ProductName.toLowerCase().includes(data.toLowerCase()))) {
          this.PendingPayments.push(this.PendingPayments1[i])
        }
      }
      this.PendingPayments
    }
    else {
      this.PendingPayments = this.PendingPayments1
    }
  }
  printreport() {
    let popupWinindow
