...
declare var $: any;
@Component({
  selector: 'app-packing-list-supplier',
  standalone: false,
  templateUrl: './packing-list-supplier.component.html',
  styleUrls: ['./packing-list-supplier.component.scss']
})
export class PackingListSupplierComponent implements OnInit {
  loginUserData: any
  supplierdata: any = []
  p: any = 1
  searchText: any
...
this.dialog.open(ErrorModalComponent, {
  ...
  totals(){
    ...
    // this.totalctns = this.productdata.map(a => Math.ceil((a.productid.piecepercarton!=0?(a.qty/a.productid.piecepercarton):0))).reduce(function (a, b) {
  ...
  gotoprint() {
    ...
    this.route.navigateByUrl('/Reports-Packing-Print?invqhid=' + this.PData.json_hdr.invqhid +'&A=GPL'); 
    console.log(this.PData.json_hdr.invqhid,"invhid")
  }
  changenw(data) {
