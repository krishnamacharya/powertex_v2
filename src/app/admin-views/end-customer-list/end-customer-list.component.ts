...
declare var $;
@Component({
  selector: 'app-end-customer-list',
  standalone: false,
  templateUrl: './end-customer-list.component.html',
  styleUrls: ['./end-customer-list.component.scss']
})
export class EndCustomerListComponent implements OnInit {
  regList: any = [];
  p: any = 1;
  page: any = 1
  pagee: any = 1;
  ...
  explore(reg) {
    ...
    this.profileService.getDatawithMethodParams1("market/outstanding_list", reg.email).subscribe((data) => {
      ...
      ($('#dealeroutstandingModal') as any).modal('show');
    })
  }
  registerusers: any
  getRegisterdUsers() {
    this.spinner.show();
    this.profileService.getDatawithQueryParams1nd4new('A', this.loginUserData.company_code,'Customer').subscribe((data) => {
      this.spinner.hide();
      console.log(data)
    ...
    // this.profileService.getDatawithQueryParams1nd4(3.5, 'A', this.loginUserData.company_code).subscribe((data) => {
    //   this.spinner.hide();
    //   this.registerusers = data
    //   if (this.registerusers.length > 0)
    //     this.regList = data;
...
this.dialog.open(ErrorModalComponent, {
  ...
  //     this.rejData = data;
  //     this.spinner.show();
  //     this.profileService.getDataOnlyWithMethod('rejectionquestions').subscribe((resp) => {
  //       this.spinner.hide();
  //       this.RejectQns=resp;
  ...
  //   this. obj={}
  //  this.count=0;
  //     for(var qn of this.RejectQns)
  //     if(qn.answer==undefined){
  //      this.count ++;
  //     }
  //     {
  //     this.obj['q'+qn['qno']]=qn.qno;
  //     this.obj['a'+qn['qno']]=qn.answer;
  //     }
  //     this.obj['rejectdby']=this.loginUserData.user_id;
  //     this.obj['rejected']=this.rejData.user_id;
  //     if(this.count<=0){
  //     this.spinner.show();
  //     this.profileService.postData(this.obj,'rejectionquestions/').subscribe((resp) => {
  //       this.spinner.hide();
  //       // this.RejectQns=resp;
  //   });
  //  // this.spinner.show();
...
//     });
  ...
  //   }
  //   else{
  //     this.toasterService.warning("Answer All the Questions")    
  //   }
  //   }
  confirm() {
...
this.dialog.open(ErrorModalComponent, {
  ...
  showLedger(reg) {
    ...
    return this.profileService.getDatawithMethodParams3("grid/", companycode,this.fromdates, this.tilldates).subscribe(data => {
      ...
      ($('#customerledger') as any).modal('show');
    })
  }
  DrTotal: number = 0;
  CrTotal: number = 0;
  ...
  printreport() {
    ...
    ($('#customerledger') as any).modal('hide');
