...
declare  var $: any;
@Component({
  selector: 'app-admin-proforma-invoice',
  standalone: false,
  templateUrl: './admin-proforma-invoice.component.html',
  styleUrls: ['./admin-proforma-invoice.component.scss']
})
export class AdminProformaInvoiceComponent implements OnInit {
  loginUserData: any
  productdata: any = []
  invoicedata: any
  pro: any = {}
...
this.dialog.open(ErrorModalComponent, {
  ...
  updatemodal() {
    ...
    this.route.navigateByUrl('/Supplier-Inv-Print?invqhid=' + this.PData.json_hdr.invqhid +'&A=GIL');
  }
  add_disable(attr) {
  ...
  AddProduct(){
    this.route.navigateByUrl('/Admin-PURCHASE ORDER?A=ITP');
  }
  errmsg: boolean = false
  checkinv(invno) {
    this.body = { 'inv_no': invno , 'user_id' : this.productdata[0].poqhid.supplierid.user_id } 
