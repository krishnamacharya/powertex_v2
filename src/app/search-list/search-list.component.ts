...
export class SearchListComponent implements OnInit {
  ...
  getdata1() {
    this.spinner.show();
    return this.service
      .getDatawithQueryParams4('10', this.d, this.e, this.select, this.modal)
      .subscribe(
        (resp) => {
          this.spinner.hide();
  ...
  get sortedFilteredProducts() {
    let filtered = this.products.filter(
      (p) => p.mrp >= this.minPrice && p.mrp <= this.maxPrice
    );
    ...
    return filtered.sort((a, b) =>
      this.sortOrder === 'asc' ? a.mrp - b.mrp : b.mrp - a.mrp
    );
  }
